(function ($) {
  ...
  $.widget("cp.UseyourDriveUploadbox", {
    ...
    _create: function () {
      ...
      this.form_submit_button = this.form_element.find('input[type="submit"]:visible, input[type="button"].gform_next_button:visible, button[id^="gform_submit_button"]:visible, button[class*="cf7md-submit-btn"]:visible, button.wpforms-submit:visible, button.wpforms-page-next:visible').not('.wpcp-upload-submit');
    ...
    _initiate: function () {
      ...
      self._log('UPLOAD BOX: LOADED', self);

      self._initUploadBox();
      self._initFunctions();
      self._refreshView();

      self._log('UPLOAD BOX: ACTIVATED', self);
    ...
    _add: function (data) {
      ...
      $.each(data.files, function (index, file) {
        ...
        self._log('UPLOAD BOX: FILE ADDED');
      });

      /* Set upload trigger */
      self.element.one('useyourdrive-upload-start', function () {
      ...
      if (self.options.auto_upload) {
        window.setTimeout(function () {
          self._log('UPLOAD BOX: AUTO UPLOAD');
    ...
    _validate: function (file) {
      ...
      // Check Quota
      self._log('UPLOAD BOX: VALIDATED', file);
      return file;
    ...
    _start: function (data) {
      ...
      self._log('UPLOAD BOX: START', data);
      ...
      if (self.upload_process === null) {
        ...
        $.when(self.upload_process).then(
          function () {
            self._log('UPLOAD BOX: ALL UPLOADS FINISHED', data);
      ...
      setTimeout(function () {
        self._preProcess().then(function () {
          ...
          self._log('UPLOAD BOX: PREPROCESS COMPLETED');
          ...
          if (self.options.support_xhr) {
            ...
            self._getUploadUrl(file).then(
              function (response) {
                ...
                self._log('UPLOAD BOX: UPLOAD DIRECTLY TO CLOUD', data);
              },
              function (response) {
                // Failed Request
                self.queue[file.hash].error = self.options.str_uploading_failed;
            ...
            self._log('UPLOAD BOX: UPLOAD TO SERVER');
            data.jqXHR = self.queue[file.hash].status.request = self.upload_box.fileupload('send', data);
          };
        }, function () {
          // Pre process failed
          self.pre_process = null;
          self.pre_process_completed = false;
          self._log('UPLOAD BOX: PREPROCESS FAILED');
    ...
    _preProcess: function () {
      ...
      self._log('UPLOAD BOX: PREPROCESS STARTED', self.pre_process);
      return self.pre_process;
    },
    ...
    _getUploadUrl: function (file) {
      ...
      self._log('UPLOAD BOX: OBTAIN DIRECT UPLOAD URL', file);
    ...
    _convert: function (file) {
      ...
      self._log('UPLOAD BOX: START CONVERT', file);
    ...
    _isFinished: function (file) {
      ...
      self._log('UPLOAD BOX: FILE UPLOAD FINISHED', file);
    ...
    _postProcess: function () {
      ...
      self._log('UPLOAD BOX: POST PROCESS STARTED');
    ...
    _finish: function (data, response) {
      ...
      self._log('UPLOAD BOX: POST PROCESS FINISHED', response);
    ...
    _watchServerProgres: function (file) {
      ...
      self._getServerProgress(file).then(function (response) {
        ...
        self._log('UPLOAD BOX: RECEIVED STATUS UPLOAD SERVER -> CLOUD', response);
    ...
    _getServerProgress: function (file) {
      ...
      self._log('UPLOAD BOX: GET STATUS UPLOAD SERVER -> CLOUD', file);
    ...
    _stop: function (file) {
      ...
      self._log('UPLOAD BOX: ABORT UPLOAD', file);
    ...
    _clear: function () {
      ...
      self._log('UPLOAD BOX: CLEAR UPLOAD BOX');
    },
    ...
    _initFormActions: function () {
      ...
      self.options.auto_upload = self.element.hasClass('force_auto_upload') || false;
    ...
    _initCustomFolderNames: function () {
      ...
      var inputs = self.form_element.find('input.useyourdrive_private_folder_name, .useyourdrive_private_folder_name input:last');
