...
//                foreach ($parents as $cached_parent) {
//                    $this->get_processor()->get_client()->update_expired_folder($cached_parent);
//                }
//                return $node;
    ...
    public function pull_for_changes($force_update = false, $buffer = 10)
    {
        ...
        if (true === $force && ($this->get_last_check_for_update() > $current_time - $buffer)) { // Don't pull again if the request was within $buffer seconds
            return false;
        }
