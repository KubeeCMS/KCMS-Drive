...
//Check if user is allowed to add folder
                ...
                if (false === $user_can_addfolder) {
                    echo json_encode(['result' => '-1', 'msg' => __('Failed to add folder', 'wpcloudplugins')]);
                    die();
                }
...
//Strip unsafe characters
                ...
                if (is_wp_error($file)) {
                    ...
                    echo json_encode(['result' => '1', 'msg' => __('Folder', 'wpcloudplugins').' '.$newfolder.' '.__('was added', 'wpcloudplugins'), 'lastFolder' => $file->get_id()]);
                }
                die();
            ...
            default:
                ...
                die('Use-your-Drive: '.__('No valid AJAX request', 'wpcloudplugins'));
        }
    ...
    public function do_shutdown()
    {
        ...
        if (isset($error['file']) && false !== strpos($error['file'], USEYOURDRIVE_ROOTDIR)) {
            error_log('[WP Cloud Plugin message]: Complete reset. Reason: '.var_export($error, true));
