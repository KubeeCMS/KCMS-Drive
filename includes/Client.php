...
class Client
{
    ...
    public $apilistchangesfields = 'changes(file(capabilities(canEdit,canRename,canDelete,canShare,canTrash,canCopy),shared,description,fileExtension,iconLink,id,imageMediaMetadata(height,rotation,width,time),mimeType,createdTime,modifiedTime,name,ownedByMe,parents,size,thumbnailLink,trashed,videoMediaMetadata(height,width,durationMillis),webContentLink,webViewLink,exportLinks,permissions(id,type,role,domain),copyRequiresWriterPermission, shortcutDetails),removed, fileId),newStartPageToken,nextPageToken';
    ...
    public function update_expired_folder(CacheNode $cachedentry)
    {
        ...
        if ('sharedfolder' === $cachedentry->get_id()) {
            $shared_folder = $this->get_shared_contents(false, true);
    ...
    public function get_root_folder()
    {
        ...
        if (true === $this->useteamfolders) {
            ...
            $team_drives_api->setName(__('Team Drives', 'wpcloudplugins'));
    ...
    public function get_folder($folderid = false, $checkauthorized = true)
    {
        ...
        if (!$cachedfolder) {
            ...
            if ($cached_team_drive = $this->get_cache()->get_node_by_id($folder_entry->get_id())) {
                if ($cached_team_drive->has_entry() && 'teamdrive' === $cached_team_drive->get_entry()->get_special_folder()) {
    ...
    public function get_shared_contents($checkauthorized = true, $isupdating = false)
    {
        $cachedfolder = ($isupdating) ? false : $this->get_cache()->is_cached('sharedfolder', 'id', false);
    ...
    public function get_team_drives()
    {
        ...
        return ['folder' => $cached_team_drives_folder, 'contents' => $team_drives_in_folder];
    }
    ...
    public function search_by_name($query)
    {
        ...
        // Remove Root folder
        unset($folders_to_look_in[0], $folders_to_look_in['team-drives']);
        ...
        if ($this->useteamfolders && $cachedfolder->is_in_folder('team-drives')) {
            if ('team-drives' === $cachedfolder->get_id()) {
    ...
    public function add_folder($new_folder_name = null)
    {
        ...
        if (null === $new_folder_name && '1' === $this->get_processor()->get_shortcode_option('debug')) {
            return new \WP_Error('broke', __('No new foldername set', 'wpcloudplugins'));
        }
        ...
        if (!$this->get_processor()->_is_entry_authorized($cachedentry)) {
            return new \WP_Error('broke', __('You are not authorized to add folders in this directory', 'wpcloudplugins'));
        }
        ...
        if (!$currentfolder->get_permission('canadd')) {
            return new \WP_Error('broke', __('You are not authorized to add a folder', 'wpcloudplugins'));
        }
        ...
        $newfolder->setParents([$currentfolder->get_id()]);

        try {
            $api_entry = $this->get_app()->get_drive()->files->create($newfolder, ['fields' => $this->apifilefields, 'userIp' => $this->_user_ip, 'supportsAllDrives' => true, 'enforceSingleParent' => true]);
    ...
    public function get_embed_url(CacheNode $cachedentry)
    {
        ...
        switch ($mimetype) {
            ...
            case 'application/vnd.google-apps.form':
                ...
                break;
            default:
                $preview = 'https://docs.google.com/file/d/'.$cachedentry->get_id().'/preview?rm=minimal';
