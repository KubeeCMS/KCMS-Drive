...
class Filebrowser
{
    ...
    public function renderFilelist()
    {
        ...
        $response = json_encode([
            ...
            'virtual' => false === $this->_search && $this->_folder['folder']->get_entry()->is_special_folder() && 'mydrive' !== $this->_folder['folder']->get_entry()->get_special_folder(),
    ...
    public function renderDir(EntryAbstract $item)
    {
        ...
        $thumburl = $item->get_thumbnail_small();
    ...
    public function renderFile(EntryAbstract $item)
    {
        ...
        $return .= "<img referrerpolicy='no-referrer' class='preloading hidden' src='".USEYOURDRIVE_ROOTPATH."/css/images/transparant.png' data-src='".$thumbnail_small."' data-src-retina='".$thumbnail_small."' data-src-backup='".str_replace('/64/', '/256/', $item->get_icon())."'/>";
        $return .= "</div></div></div>\n";
    ...
    public function renderFileNameLink(EntryAbstract $item)
    {
        ...
        if (('0' !== $this->get_processor()->get_shortcode_option('mcepopup')) || (!$usercanpreview)) {
            if ($usercanread) {
                $url = USEYOURDRIVE_ADMIN_URL."?action=useyourdrive-download&account_id={$this->_folder['folder']->get_account_id()}&id=".$item->get_id().'&dl=1&listtoken='.$this->get_processor()->get_listtoken();
            ...
            if (in_array($item->get_extension(), ['jpg', 'jpeg', 'gif', 'png'])) {
                ...
                //$datatype = 'inline';
                //$url = USEYOURDRIVE_ADMIN_URL . "?action=useyourdrive-stream&account_id={$this->_folder['folder']->get_account_id()}&id=" . $item->get_id() . "&dl=0&listtoken=" . $this->get_processor()->get_listtoken();
            }
        } elseif ($usercanpreview && $item->get_can_preview_by_cloud()) {
            // If user can't dowload a file or can preview and file can be previewd
            $url = USEYOURDRIVE_ADMIN_URL."?action=useyourdrive-preview&account_id={$this->_folder['folder']->get_account_id()}&id=".urlencode($item->get_id()).'&listtoken='.$this->get_processor()->get_listtoken();
    ...
    public function renderActionMenu(EntryAbstract $item)
    {
        ...
        if (($usercanread) && ($item->is_file()) && (0 === count($item->get_save_as()))) {
            $html .= "<li><a href='".USEYOURDRIVE_ADMIN_URL."?action=useyourdrive-download&account_id={$this->_folder['folder']->get_account_id()}&id=".$item->get_id().'&dl=1&listtoken='.$this->get_processor()->get_listtoken()."' class='entry_action_download' download='".$item->get_name()."' data-filename='".$filename."' title='".__('Download', 'wpcloudplugins')."'><i class='fas fa-arrow-down'></i>&nbsp;".__('Download', 'wpcloudplugins').'</a></li>';
        ...
        if (($usercanedit) && ($item->is_file()) && $item->get_can_edit_by_cloud()) {
            $html .= "<li><a href='".USEYOURDRIVE_ADMIN_URL."?action=useyourdrive-edit&account_id={$this->_folder['folder']->get_account_id()}&id=".$item->get_id().'&listtoken='.$this->get_processor()->get_listtoken()."' target='_blank' class='entry_action_edit' data-filename='".$filename."' title='".__('Edit (new window)', 'wpcloudplugins')."'><i class='fas fa-pen-square'></i>&nbsp;".__('Edit (new window)', 'wpcloudplugins').'</a></li>';
        }
