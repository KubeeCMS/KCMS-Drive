    ...
    public function createMediaArray()
    {
            ...
            foreach ($this->_folder['contents'] as $key => $node) {
                ...
                if (in_array(strtolower($child->extension), ['png', 'jpg', 'jpeg'])) {
                    ...
                    $captions[$video_name][] = [
                        ...
                        'src' => USEYOURDRIVE_ADMIN_URL."?action=useyourdrive-stream&account_id={$this->_folder['folder']->get_account_id()}&id=".$child->get_id().'&dl=1&caption=1&listtoken='.$this->get_processor()->get_listtoken(),
                    ];
        ...
        if (count($this->_folder['contents']) > 0) {
            ...
            foreach ($this->_folder['contents'] as $node) {
                ...
                if (!isset($files[$path])) {
                    $source_url = USEYOURDRIVE_ADMIN_URL."?action=useyourdrive-stream&account_id={$this->_folder['folder']->get_account_id()}&id=".$child->get_id().'&dl=1&listtoken='.$this->get_processor()->get_listtoken();
